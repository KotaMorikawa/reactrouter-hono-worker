...
export function Alert({ type, title, message, onClose, className = "" }: AlertProps) {
	...
	return (
		<div className={`${baseClasses} ${typeClasses[type]} ${className}`} role="alert">
			<div className="flex">
				<div className="flex-shrink-0">{iconElements[type]}</div>
				...
				{onClose && (
					<div className="ml-auto pl-3">
						<button
							type="button"
							onClick={onClose}
							className="inline-flex rounded-md p-1.5 hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-current focus:ring-offset-2"
						>
...
export function Toast({ isVisible, onClose, duration = 5000, ...alertProps }: ToastProps) {
	React.useEffect(() => {
